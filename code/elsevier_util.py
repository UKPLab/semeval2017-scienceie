...
def parseXML_text(fpath="data/dev/S0010938X13003818.xml"):
    '''
    Parse XML files to retrieve full publication text
    :param fpath: path to file
    :return:
    ...
    with codecs.open(outFile, "w", "utf-8") as outF:
        for f in dir:
            if not f.endswith(".xml"):
                continue
            print(f)
            outF.write("\n".join(parseXML_text(os.path.join(dirpath, f))))
